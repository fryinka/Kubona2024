...
import { Prodlist, DepartmentGroup, SizeGroup, ColorsGroup, StylesGroup, IComplexItem, CategoryTitle, HeelHeightGroup, MaterialGroup, PriceRange, ProductImages } from '../models/prod-list';
...
export class ProdListComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.productService.getSample().subscribe(res => {
          this.data = res
          this.prodlists = this.data;
        });
    ...
    this.activateRoute.paramMap.pipe(switchMap((params: ParamMap) =>
      this.productService.getDepartmentGroupBy<DepartmentGroup>(params.get('departmentId'))
       ...
       this.deptGrplist = result;
       this.prodImages = this.deptGrplist.map((deptGroup: DepartmentGroup) => ({
        orderId: deptGroup.departmentId,
        image: '',
        thumbImage: `../../assets/${this.getImageFileName(deptGroup.description)}`,
        alt: '', 
        title: ''
      }));
       }, error => console.error(error));
  ...
  // trackByFn(index:number, item: Prodlist) {
  ...
  openLink(e:any){
    const deptId=this.deptGrplist[e].destinationUrl;
    this.googleService.searchListEventEmitter("filter_links", "search_page", this.deptGrplist[e].description);
    this.router.navigate(['/category',deptId]);
  }

  getImageFileName(imageTitle: string): string {
    // Example logic: Replace spaces with underscores and convert to lowercase
    return imageTitle.replace(/\s+/g, '_').toLowerCase() + '.png';
  }
