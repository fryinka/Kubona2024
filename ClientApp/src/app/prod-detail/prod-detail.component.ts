...
export class ProdDetailComponent implements OnInit {
  ...
  public tempProdImages: ProductImages[] = [{
    orderId: 0,
    image: '../../assets/Loading.webp',
    thumbImage: '../../assets/Loading.webp',
    alt: 'Loading...',
    title: 'Loading...'
  },
  {
  ...
  ngOnInit() {
    ...
    this.prodImages = this.tempProdImages;
    this.activateRoute.paramMap.pipe(
      switchMap((params: ParamMap) =>
        this.productService.getProductImages<ProductImages>(params.get('productId'))
      )).subscribe(result => {
      this.prodImages = this.tempProdImages;
      if (result && result.length > 0) {
        this.prodImages = result;
      }
    }, error => {
      console.error(error);
    });
  ...
  addProductToOrder() {
    ...
    if (this.checkoutForm.invalid) {
      ...
      if (this.checkoutForm.valid) {
        ...
        if (this.productId > 0) {
          if (this.itemgroupSizeId >= 0) {
            this.orderService.insertItemIntoOrder(this.productId, this.itemgroupSizeId).subscribe(result => {
                this.orderId = result;
                if (this.orderId > 0) {
                  this.isDisabled = true;
